...
class Network:
    ...
    def BackwardPass(self, Input, desired):
        ...
        self.W3 += (self.hidout2.T.dot(out_delta) * self.lrate - self.lrate * self.alpha * self.W3)
        self.B3 += (-1 * self.lrate * out_delta)
        self.W2 += (self.hidout1.T.dot(hid_delta2) * self.lrate - self.lrate * self.alpha * self.W2)
        self.B2 += (-1 * self.lrate * hid_delta2)
        self.W1 += (Input.T.dot(hid_delta1) * self.lrate - self.lrate * self.alpha * self.W1)
...
if __name__ == '__main__':
    ...
    for epoch in range(minepoch, maxepoch, inc):
        ...
        network_red.BP_GD(stocastic=True, vanilla=1, depth=epoch)

        print "\nTrain Data performance: "
        sse, acc = network_red.TestNetwork(phase=0, erTolerance=etol_tr)
        print("sse: "+ str(sse) + " acc: "+ str(acc))
        train_acc_wo.append(acc)
        train_sse_wo.append(sse)

        print "Test Data performance: "
        sse, acc = network_red.TestNetwork(phase=1, erTolerance=etol)
        print("sse: " + str(sse) + " acc: " + str(acc))
        test_acc_wo.append(acc)
        test_sse_wo.append(acc)
        ...
        network_red_trf.BP_GD(stocastic=True, vanilla=1, depth=epoch)
        ...
        train_sse_w.append(sse)

        print "\nTest Data performance: "
        sse, acc = network_red_trf.TestNetwork(phase=1, erTolerance=etol)
        print("sse: " + str(sse) + " acc: " + str(acc))
        test_sse_w.append(sse)
        test_acc_w.append(acc)
    ...
    ax = plt.subplot(111)
    plt.plot(range(minepoch, maxepoch, inc), train_acc_w, label="train_w_trf")
    plt.plot(range(minepoch, maxepoch, inc), test_acc_w, label="test_w_trf")
    plt.plot(range(minepoch, maxepoch, inc), train_acc_wo, label="train_wo_trf")
    plt.plot(range(minepoch, maxepoch, inc), test_acc_wo, label="test_wo_trf")
