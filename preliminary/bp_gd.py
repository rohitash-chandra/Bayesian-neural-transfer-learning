...
class MTnetwork:  # Multi-Task leaning using Stocastic GD
    ...
    def transferKnowledge(self, Wprev,
                          Wnext):  # transfer knowledge (weights from given layer) from  Task n (Wprev) to Task n+1 (Wnext)
        x = 0
        y = 0
    ...
    def mainAlg(self):
        ...
        while (current) < (self.maxTime):  # *self.numModules
            ...
            for n in xrange(0, self.numModules - 1):
                ...
                (erPlot[index, n], trainMSE[n], trainPerf[n], Epochs) = Netlist[n].BP_GD(self.learnRate, mRate,
                                                                                         stocastic, vanilla,
                                                                                         depthSearch)
            ...
            (
            erPlot[index, self.numModules - 1], trainMSE[self.numModules - 1], trainPerf[self.numModules - 1], Epochs) = \
            Netlist[self.numModules - 1].BP_GD(self.learnRate, mRate, stocastic, vanilla,
                                               depthSearch)  # BP for last module
            index = index + 1
            # print trainPerf, cycles
        for n in xrange(0, self.numModules):
            (testMSE[n], testPerf[n]) = Netlist[n].TestNetwork(1,
                                                               self.testTolerance)  # 1 in argument means to use testdata
...
def main():
    ...
    moduledecomp = [0.25, 0.5, 0.75,
                    1]  # decide what will be number of features for each group of taskdata correpond to module
    ...
    for problem in range(0, 13):
        ...
        for i in xrange(1, numModules):
            ...
            mtaskNet[i][1] += (
                        i * 2)  # in this example, we have fixed numner  output neurons. input for each task is termined by feature group size.
        ...
        for transKnow in xrange(1,
                                2):  # transKnow = 0 # 1 is for MT knowledge transfer. 0 is for no transfer (simple ensemble learning)
